...
with tab2:
    ...
    if workers_input and minutes_input and efficiency_input:
        ...
        with col_calc1:
            st.metric("💪 Capacidade Nominal", f"{nominal_capacity:.0f} min/dia", f"{nominal_capacity/60:.1f} h/dia")
        
        with col_calc2:
            st.metric("⚡ Capacidade Efetiva", f"{effective_capacity:.0f} min/dia", f"{effective_capacity/60:.1f} h/dia")
        
        with col_calc3:
            st.metric("📈 Minutos Efetivos", f"{effective_minutes:.0f} min", "por trabalhador/dia")
...
with tab4:
    ...
    with col1:
        blocked_date = st.date_input("Selecione a data", value=datetime.now().date())
...
with tab5:
    ...
    with col1:
        ...
        if st.session_state.orders:
            ...
            st.download_button("📥 Download Pedidos (CSV)", csv, f"pedidos_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv")
        else:
    ...
    with col2:
        ...
        if st.session_state.parts:
            ...
            csv = df.to_csv(index=False, encoding='utf-8')
            st.download_button("📥 Download Peças (CSV)", csv, f"pecas_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv")
        else:
            st.info("Nenhuma peça para exportar")

st.markdown("---")
st.markdown('<div style="text-align: center; color: gray;"><p>Sistema de Agendamento de Produção v6.0 🚀</p></div>', unsafe_allow_html=True)